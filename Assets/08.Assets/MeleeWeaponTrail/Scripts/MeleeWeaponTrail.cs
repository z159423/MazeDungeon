...
#pragma warning disable 0649
	...
	[SerializeField]
	public Color[] _colors = new Color[1];
	...
	void Start()
	{
        ...
        if (_material == null)
            _material = Resources.Load("Mat/meleeTrail") as Material;
        ...
        _colors[0] = new Color(135 / 255f, 135 / 255f, 135 / 255f, 135 / 255f);
	...
	void Update()
	{
		...
		if (_emit)
		{
			if (theDistanceSqr > _minVertexDistanceSqr)
			{
				...
				{
					Point p = new Point();
					p.basePosition = _start.position;
					p.tipPosition = _end.position;
				...
				{
					_points[_points.Count - 1].basePosition = _start.position;
					_points[_points.Count - 1].tipPosition = _end.position;
					//_points[_points.Count - 1].timeCreated = Time.time;

#if USE_INTERPOLATION
					_smoothedPoints[_smoothedPoints.Count - 1].basePosition = _start.position;
					_smoothedPoints[_smoothedPoints.Count - 1].tipPosition = _end.position;
#endif
				}
			}
			else
